...
def generate_cluster(cluster_result_path, dist=None, eps=0.8, min_samples=10, data_dir=None, flag='did',x=None,y=None,cam_num=6):
    ...
    xtmp = x * -0.99 + y
    disttmp = dist + 0.3* xtmp
    # disttmp= dist
    if dist is None:
        X = target_features[:process_num]
    else:

        X = disttmp[:process_num]
    ...
    for i in np.arange(n_clusters_):
        ...
        if len(files) > np.max((2, min_samples - 1)):
            ...
            for file in files:
                file = file.strip()
                cam= file.split('c')[1][0]
                shutil.copy(
                    os.path.join(os.path.split(os.path.split(cluster_result_path)[0])[0], 'bounding_box_train', file),
                    os.path.join(dir_path, file))
                maskfile = file[:-4] + '_mask' + '.jpg'
                
                shutil.copy(

                    os.path.join(os.path.split(os.path.split(cluster_result_path)[0])[0], 'bounding_box_train', maskfile),
                    os.path.join(dir_path, maskfile))
            
                for k in range(cam_num):
                  if k+1 != int(cam):
                    cam_path=file[:-4] + '_fake_' + str(cam) + 'to' + str(k+ 1) + '.jpg'
                    shutil.copy(
                     os.path.join(os.path.split(os.path.split(cluster_result_path)[0])[0], 'bounding_box_train', cam_path),
                     os.path.join(dir_path, cam_path))
        dir_cnt += 1
        image_cnt += len(files)
