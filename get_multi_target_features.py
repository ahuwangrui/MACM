...
import numpy as np
from datasets import  ChannelTripletFolder
...
def get_features(flag='did', multi_domain=False, order=0, data_dir=None, net_loss_model=None, domain_num=None, which_epoch=None):
    ...
    print('---------test-----------')
    class_num = len(os.listdir(os.path.join(data_dir, 'train_ori')))
    sid_num = class_num
...
def get_distances(src_path, tgt_path, order=-1, ratio=0.003, domain=0, flag='all'):
    ...
    camtarget = m['train_cam']
    x = np.array(camtarget)
    x = np.transpose(x)
    # x=x[:,np.newaxis]
    x = cdist(x, x, 'jaccard')
    shapex = x.shape
    y = np.ones(shapex)
    rerank_dist = re_ranking(source_features, target_features, lambda_value=0.1,x=x,y=y)
    # x = np.array(camtarget)
    # x = np.transpose(x)
    # # x=x[:,np.newaxis]
    # x = cdist(x, x, 'jaccard')
