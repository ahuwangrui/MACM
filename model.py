...
def load_network2(network, name, model_name=None):
    if model_name == None:
        save_path = os.path.join('./modelnew', name, 'net_%s.pth' % 'last')
    else:
        save_path = os.path.join('./modelnew', name, 'net_%s.pth' % model_name)
    print('load easy pretrained model: %s' % save_path)
    network.load_state_dict(torch.load(save_path))
    return network
...
def load_whole_network2(network, name, model_name=None):
    if model_name == None:
        save_path = os.path.join('./modelnew', name, 'net_%s.pth' % 'whole_last')
    else:
        save_path = os.path.join('./modelnew', name, 'net_%s.pth' % model_name)
    print('load whole pretrained model: %s' % save_path)
    net_original = torch.load(save_path)
...
def save_network2(network, name, epoch_label):
    save_filename = 'net_%s.pth' % epoch_label
    save_path = os.path.join('./modelnew', name, save_filename)
    torch.save(network.state_dict(), save_path)
...
def save_whole_network2(network, name, epoch_label):
    save_filename = 'net_%s.pth' % epoch_label
    save_path = os.path.join('./modelnew', name, save_filename)
    torch.save(network, save_path)
